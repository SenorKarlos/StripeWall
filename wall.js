...
const migration = require(__dirname+"/modules/migration.js");
...
// Function to initialize everything in sequence
async function initialize() {
  // Perform migration
  await migration.start();

  // Check or create service zones if enabled
  if (config.service_zones.zones_enabled) {
    console.info("["+utils.getTime("stamp")+"] [wall.js] Check or Create Service Zone Table.");
    let service_zones = await database.fetchZones();
    if (!service_zones || service_zones.length === 0) {
      console.info("["+utils.getTime("stamp")+"] [wall.js] No Service Zone Table Present, Create.");
      let geojson;
...
function startServer() {
  ...
  // Send to Discord OAUTH2 to get code
    if (!req.query.code) {
      console.info("["+utils.getTime("stamp")+"] [wall.js] Login from "+req.headers['x-forwarded-for']+". Sending User to Discord Oauth2 Authorization URL.");
  ...
  // Fetch discord tokens with code
      try {
        ...
        req.session = null;
        console.info("["+utils.getTime("stamp")+"] [wall.js] Failed to fetch Tokens", e);
      ...
      try {
        ...
        req.session = null;
        console.info("["+utils.getTime("stamp")+"] [wall.js] Failed to fetch User from Oauth2", e);
        return res.redirect(`/error`);
      }
  ...
  // Check login vs blacklist
      try {
        if (bot.blacklisted.indexOf(user.id) >= 0) {
          console.info("["+utils.getTime("stamp")+"] [wall.js] Blacklist Login Attempt.");
          bot.sendEmbed(user.username, user.id, "FF0000", "Blacklist Login Attempt", "", config.discord.log_channel);
          return res.redirect(`/blocked`);
        }
        else { 
          console.info("["+utils.getTime("stamp")+"] [wall.js] User Passed Blacklist");
        }
      } catch (e) {
        req.session = null;
        console.info("["+utils.getTime("stamp")+"] [wall.js] Blacklist Check Failure", e);
        return res.redirect(`/error`)
      }
  ...
  // Check guild membership and add if required
      try {
        ...
        req.session = null;
        console.info("["+utils.getTime("stamp")+"] [wall.js] Guild Member Fetch Failure", e);
        return res.redirect(`/error`)
      }
      if (!member) {
        try {
          console.info("["+utils.getTime("stamp")+"] [wall.js] "+user.username+" not a Guild Member, adding.");
          await oauth2.joinGuild(data.access_token, config.discord.guild_id, user.id);
        } catch (e) {
          req.session = null;
          console.info("["+utils.getTime("stamp")+"] [wall.js] Failed to join User to Guild", e);
          return res.redirect(`/error`);
        }
      }
  ...
  // Insert/update user and pull current record
      try {
        ...
        req.session = null;
        console.info("["+utils.getTime("stamp")+"] [wall.js] Failed to Insert/Update Database User", e);
        return res.redirect(`/error`);
      }
      ...
      try {
        ...
        req.session = null;
        console.info("["+utils.getTime("stamp")+"] [wall.js] Failed to fetch Database User", e);
        return res.redirect(`/error`);
      }
  ...
  // Verify or Create Stripe Customer
      if (!dbuser.stripe_data) {
        try {
          ...
          req.session = null;
          console.info("["+utils.getTime("stamp")+"] [wall.js] Failed to Create Stripe Customer", e);
          return res.redirect(`/error`);
        }
      }
      if (dbuser.stripe_data && dbuser.stripe_data != 'ERROR') {
        ...
        if (dbuser.email != dbuser.stripe_data.email || dbuser.user_name != dbuser.stripe_data.name) {
          try {
            ...
            req.session = null;
            console.info("["+utils.getTime("stamp")+"] [wall.js] Failed to Update Stripe Customer", e);
            return res.redirect(`/error`);
          }
        }
      }
      ...
      if (config.qbo.enabled) {
        if (!dbuser.qbo_data) {
          try {
            ...
            req.session = null;
            console.info("["+utils.getTime("stamp")+"] [wall.js] Failed to Create QBO Customer", e);
            return res.redirect(`/error`);
          }
        }
        if (dbuser.qbo_data) {
          ...
          if (dbuser.email != dbuser.qbo_data.PrimaryEmailAddr.Address || dbuser.user_name != dbuser.qbo_data.GivenName || dbuser.user_name != dbuser.qbo_data.DisplayName || dbuser.user_name != dbuser.qbo_data.PrintOnCheckName) {
            try {
              ...
              req.session = null;
              console.info("["+utils.getTime("stamp")+"] [wall.js] Failed to Update QBO Customer", e);
              return res.redirect(`/error`);
            }
          }
        }
  ...
  server.get("/terms", async function(req, res) {
    ...
    if (!req.session.login || unix > req.session.now+1800) {
      console.info("["+utils.getTime("stamp")+"] [wall.js] Direct Link Accessed, Sending to Login");
     return res.redirect(`https://discord.com/api/oauth2/authorize?response_type=code&client_id=${oauth2.client_id}&scope=${oauth2.scope}&redirect_uri=${config.discord.redirect_url}`);
    }
  ...
  server.get("/manage", async function(req, res) {
    let unix = moment().unix();
    if (!req.session.login || unix > req.session.now+1800) {
      console.info("["+utils.getTime("stamp")+"] [wall.js] Direct Link Accessed or Data 'Old' -  Sending to Login");
      return res.redirect(`https://discord.com/api/oauth2/authorize?response_type=code&client_id=${oauth2.client_id}&scope=${oauth2.scope}&redirect_uri=${config.discord.redirect_url}`);
    }
    let dbuser = await database.fetchUser(req.session.discord_id);
    if (dbuser.terms_reviewed == 'false') {
  ...
  server.listen(config.server.listening_port, () => {
    console.info("[" + utils.getTime("stamp") + "] [wall.js] Now Listening on port " + config.server.listening_port + ".");
  });
}

// Call initialize function to start the initialization process
initialize();